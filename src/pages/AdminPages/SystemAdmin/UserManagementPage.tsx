...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ isEmbedded = false }) => {
  ...
  useEffect(() => {
    ...
    // Only fetch users on initial load and page changes
  ...
  }, [currentPage, limit]) // Remove navigate and isAdmin from dependencies

  //useEffect for search
  useEffect(() => {
    if (!debouncedSearchTerm) {
      if (searchTerm === '' && !isLoading && !isSearching) {
        // Only refetch if we're not already loading and the search term was cleared
        fetchUsers()
      }
      return
    }
    ...
    fetchSearchResults()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedSearchTerm, currentPage, limit]) // Remove navigate from dependencies
